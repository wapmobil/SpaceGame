...
func handleGetBuildDetails(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		resp := BuildDetailsResponse{
			...
			BaseLevel:               details.BaseLevel,
			CommandCenterLevel:      details.CommandCenterLevel,
			MaxSurfaceExpeditions:   details.MaxSurfaceExpeditions,
			CanResearch:             details.CanResearch,
			CanExpedition:          details.CanExpedition,
			PlanetSurveyUnlocked:   details.PlanetSurveyUnlocked,
